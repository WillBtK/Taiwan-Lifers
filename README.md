...
Both questions are answered from the same series stack (section 3). **Read decisions 4.34 first:** the sector hedge amount IS published — a footnote on the CBC's life-insurer balance sheet, monthly since 2012 — and this log's earlier statements that no hedge amount exists anywhere were wrong. Setser & S.T.W. (2019) §II.D names it, alongside the holdco-deck composite. Interpretation notes and references are in sections 9 and 10.
...
|---|--------|------------|-------|-------|-------|
| 1 | **Economic hedge ratio** (headline) | (derivative hedges + FX-denominated policy liabilities) / foreign-currency assets. **Bottom-up composite from 2014-12** (`economic_hedge_ratio_deck_composite`, Cathay + KGI, pie over the FX-risk subset x the risk bar; Fubon excluded — its wedge is over total FX assets and changes base in 2017, decisions 4.35) | sector + firm | M (sector), Q (firm) | **composite 2014-12**; firm **2014-04** (Fubon, 36 q); sector 2024-04 |
| 2 | **Net open FX position** | foreign-currency assets − FX policy liabilities − derivative hedges, stored in NT$ million. USD, % GDP, % assets and × capital are presentations of the same series, selected in the artifact by a unit toggle — never a second y-axis | sector + firm | M / Q | sector 2024-04 / firm **2014-04** |
...
| 4 | **Regulatory hedge ratio** | FSC definition (2026 notice §(九)): traditional hedge principal / (foreign investments − FX-policy liabilities − unhedged non-FVTPL equities and funds). Never printed in the monthly release (decisions 1.2); the Bureau's own portal is the source of record, the press briefing the current fallback. v1/v2 is a definitional break at the Feb 2026 notice, not a channel break | sector | M | published **2024-04** — earlier press mentions are spoken ranges, not the figure (decisions 1.11). **The notice defines it exactly (§三(九)):** 傳統避險本金金額 ÷ (國外投資總額 − 外幣收付之非投資型人身保險商品負債 − 未避險且非以透過損益按公允價值衡量之股票與基金), where 傳統避險 (§三(五)) is forwards, FX swaps, CCS and NDFs **against TWD only**, and the excluded equity/fund bucket (§三(四)) is exactly the positions whose FX revaluation misses P&L. **NOTHING RECONSTRUCTS IT.** The numerator is a NOTIONAL PRINCIPAL; the P&L identity yields a mark-to-market, and the two coincide only if every hedge marks at notional × the asset-revaluing spot move — false for 換匯 and 換匯換利. The former `reg_hedge_ratio_pl_*` series are renamed `fx_pl_offset_ratio_month` / `_roll` and are a different measure, not an estimate of this one (decisions 4.49). Two routes to a real history, both in the same notice: §三(十一) defines the 避險比率基準 as the **90th percentile of month-end ratios over 2021-2025** on the official definition, and §八's 強化準備 provision = average denominator × 避險比率差額 × 2.5%, which inverts from firm disclosures for FY2026 on |
| 5 | **Gross hedge ratio** | derivative hedges / foreign assets. **NOT the commonly-quoted figure** — that is series 4, the FSC's regulatory 匯率避險比率: the CBC's own *Financial Stability Report* (May 2026, p.80) states 「114年底壽險公司匯率避險比率降至50.23%，為歷史新低」, which ties `reg_hedge_ratio` at 2025-12 exactly, and it is the only hedge ratio any Taiwanese authority publishes (decisions 4.47). An earlier version of this line claimed the gross ratio was what sell-side and press quote; that was unsourced and the evidence contradicts it. **Sector anchor from 2012:** the CBC's own footnote on Financial Statistics Monthly appendix 8 gives the sector's outstanding swap-type hedges monthly (`hedge_outstanding_cbc_footnote`, onshore swaps, ≈62–77% of the FSC principal, so NDFs excluded); eleven editions recovered from the Internet Archive, ratio vs the same table's 國外資產 55.8% (2012-03) → 23.8% (2026-07) (decisions 4.34) | sector + firm | M / Q | **sector 2012-03** (CBC footnote, 11 points only); firm **2013-10** |
| 6 | **Hedge composition** | CS, NDF, proxy, FX policy, open — shares and NT$ | firm | Q | **2014-03** (first Cathay deck disclosing the split; Fubon 1H14) |
| 7 | **Hedge cost** | reported annualised hedging cost (bp of foreign assets) vs market implied cost from CBC USD/TWD forwards by tenor. **The two firm series are not on one definition** — Fubon's behaves as an all-in ex-ante carry (0.51x market over 26 quarters), Cathay's exceeds the market cost in 3 of 11 quarters and cannot; do not draw them as one line (decisions 4.30) | firm + market | Q / M | firm **2013-10**; **market 1992-01** |
...
| 10 | **US holdings by security type — A COUNTRY SERIES, NOT A LIFER SERIES** | Taiwan's holdings of US long-term securities from the TIC benchmark survey, split Treasuries / agency debt / agency ABS / corporate bonds / corporate ABS / equities, with the all-foreign-holder denominators stored alongside so a market share is computed rather than asserted. **No line here may be attributed to the life insurers (decisions 4.44).** TIC publishes no official/private split by country at any frequency, and the CBC held USD 554.7bn of securities at the June-2025 survey date against a Taiwan total of 677bn of US long-term debt — the official sector can account for most of the row. Taiwan-level facts that stand: peak share of all foreign-held US agency MBS **26.2% (2018)**, now 14.0%; corporate bonds 1.5% → **4.1%**; **33% of the increase in foreign agency-MBS holdings 2013-19** and **17% of the increase in foreign corporate-bond holdings 2019-24**. Levels are at market value, so a change in level is not a purchase (4.42). For lifer claims use series 9, which is sector-identified and excludes reserves by construction | **country (all holders incl. the CBC)** | A | **2013** (composition), 2007 (total LT debt) |
| 12 | **CBC balance sheet (IRFCL)** | the central bank's own quarterly IMF/BIS template: reserve assets split into securities and deposits, gold, other FX assets, and **Section II's aggregate short position in FX forwards and futures including the forward leg of currency swaps — the CBC swap book**. Securities 91-94% of reserves throughout; the swap book **USD 91.9bn (2021Q4) → 76.1bn (2026Q1)**, at the low end of Setser & S.T.W.'s 60-200bn estimated interval and shrinking. Loaded so that series 10 can be bounded rather than attributed, and as a direct input to series 8 (decisions 4.44) | central bank | Q | **2021-12** |
| 11 | **Formosa bond register** | the TPEx register of listed 國際債券 by currency, original tenor, remaining maturity, call structure and maturity ladder. USD 218bn, 92.6% of the market, **amount-weighted original tenor 28.0 years, 74.5% with 20+ years to run, 75.9% issuer-callable**. This is USD duration the TIC survey cannot see, because the issuers are not US residents. The register carries **no holder information** — it is market size, not lifer holdings (decisions 4.41) | market | snapshot | 2026-09 |
...
**Stage 3 — Firm panel (2013-10–present; the decks do not disclose the FX split earlier).** Statutory filings first, IR decks second
