...
**Stage 5 opened with the hedge-ratio reconstruction (4.29):** the regulatory ratio is now inferred from the release's own P&L lines back to **2019-05** — nineteen observations, validated against the published anchors at 4.4pp MAE and essentially unbiased (+0.8pp), loaded as `reg_hedge_ratio_pl_implied` alongside a buffer-inclusive `fx_offset_total_pl_implied`. **The market side is in (4.30):** the CBC's USD/TWD forwards, spot and FX turnover are loaded as `fx_market_monthly` (12,901 obs, migration 0015), giving a market-implied hedging cost back to **1992-01** — 2023-24 is the most expensive hedging environment in the series, which is the ordering the price story for the falling hedge ratio requires. Two counterparty results: the banking system's entire net FX position is ~0.06% of the lifers' foreign book, and the lifers' hedge rolling is the same order as the whole onshore customer swap and forward market. One caution recorded: the Fubon and Cathay hedge-cost series are not on one definition and must not be pooled. **The price explanation for the falling hedge ratio is then tested and rejected (4.31):** cost explains R² 0.23 alone, a time trend explains 0.47, and cost retains nothing once the trend is removed — the ratio's steepest fall (75.9% → 59.2%, 2024-08 to 2025-11) came as carry got *cheaper*. The decline is a steady ~2.6pp/year structural trend, which points at the reserve regime, the FX-policy push and IFRS 17. **SCOPE WIDENED (4.39): the duration-demand channel is now the primary question.** Taiwan's BoP splits foreign debt-security purchases by sector AND tenor, so the lifers' long-term bid is directly observable quarterly. Net acquisition of long-term foreign debt by other financial institutions — a sector whose IIP debt stock the lifers' own foreign assets run at 90-110% of — peaked at USD 57bn (2018) and **went NEGATIVE in 2025 at −USD 7.7bn, including −USD 12.4bn in 2025Q2**, the TWD-shock quarter and the largest liquidation in the series; 2026Q1 is negative too. The stock is USD 778bn, 71% of all Taiwanese foreign debt holdings. Taiwan is running a record current-account surplus while its main private recycler is a net seller. **The company panel is 2009-deep and the first per-firm hedge notionals are loaded (4.38):** 表06021011's period selectors offer ROC 98–115 (2009 Q1 → 2026 Q4) across 55 insurers — read straight from a page already on disk, answering the reset's first question. Reaching it needs a POST (GET returns a 3.5KB error against a 21.6KB form), so the relay gained `/post` with cookie pass-through; **that redeploy is the one action blocking the panel pull**. Shin Kong Life's statutory notionals are loaded — six quarters, 2020-03 → 2024-06, NT$1,047bn → NT$1,399bn, with the swap/forward split tying to the stated total at period-end spot on all six to four decimals. Coverage is one firm: Cathay applies hedge accounting (NT$49bn designated vs NT$5.5tn assets) and the other four are untested — MOPS lists the life subsidiaries (新光人壽 28880001, 富邦人壽 28810012, 台壽保 28910031) but its document server serves none of them. **A supplied DATA_SOURCES.md reset is adopted (4.37)** for its target variables, acceptance test, extraction rules and order of work — with three corrections: the FSC statistics API carries none of the six target variables (0 of 165 datasets match any target term), `checkrpt.aspx` is a menu page rather than the aggregation query, and the CBC keeps no per-month PDF archive. It also omits that ins-info is geo-blocked rather than robots-blocked. **The highest-value open test is now the period depth of `RPT-06021011` on the Observation Station**, which needs a POST through the Taiwan relay; the relay currently forwards GET only. **A supplied source map was verified link by link (4.36) and overturned a standing negative:** Shin Kong Life's quarterly statutory statements disclose the exact notional principal of its FX hedges (NT$1,030bn at 2021-03-31), with the swap/forward split in USD reconciling to the NT$ total at period-end spot on three dates. 4.26's "no hedge amount is published for any insurer" was true of the portal, false of the filings. Cathay's statement notional is hedge-accounting only (NT$49bn vs NT$5.5tn) and is not a substitute. Next: walk the Shin Kong filing index, test the other four firms, and patch the Cathay FY22 / Fubon 2026 extraction gaps the document exposed. **The bottom-up composite is built (4.35):** Setser's method 1, from deck data already extracted — 31 quarters, 2014-12 → 2026-06, gross 37% → 27% and economic 61% → 53%; it validates against the CBC footnote amount at a stable 0.79 across eight points, two constructions with no shared input. Caveat on the record: 28 of 31 quarters are a single firm (Cathay, KGI from 2023), so the shape carries the weight, not the level. **Correction, 4.34:** the sector hedge amount is published — a footnote on the CBC's table 8, monthly since 2012 — and had been sitting unparsed in the Stage 2 cache; eleven historical editions recovered from the Internet Archive give a sector anchor 55.8% (2012-03) → 23.8% (2026-07), against which the P&L-implied, deck and FSC series all reconcile. Separating those three now needs the FX-policy liability series, which the backfill spec lists as a genuine gap — **that gap is now the binding constraint on research question 2**, not a loose end. Remaining in Stage 5: the BIS/IRFCL legs of series 8 and the other CBC series (EG49, EG01, EG46, BPP2, BPF4) still cached but unparsed. **The composition of Taiwan's US book is measured from the US side (4.40), but it is a COUNTRY series and 4.44 WITHDRAWS every attribution of it to the lifers.** The TIC benchmark survey splits Taiwan's US holdings by security type, twelve vintages 2013–2025: Taiwan held **26.2% of ALL foreign-held US agency ABS at the 2018 peak**, USD 265bn, now 184bn and 14.0%; corporate bonds went USD 35bn and 1.5% of foreign holdings in 2013 to **186bn and 4.1% in 2025**; Taiwan absorbed **33% of the increase in foreign agency-MBS holdings 2013–2019** and **17% of the increase in foreign corporate-bond holdings 2019–2024**. All of that is true of TAIWAN, not of the lifers. **TIC publishes no official/private split by country at any frequency** (checked: the annual survey's FOI line is global only, the monthly SLT country tables have no official column, Major Foreign Holders is Treasuries-only), and the argument 4.40 used — that a reserve manager does not hold MBS or credit — is refuted by TIC's own global figures, where official institutions hold USD 524bn of US agency paper against 829bn private. **The CBC's own balance sheet, now loaded as series 12 from the quarterly IRFCL template, settles it:** reserve securities USD 554.7bn at the June-2025 survey date against a Taiwan-wide 677bn of US long-term debt, so the official sector can account for most of the row and the private residual cannot be pinned down. The sums also do not close — OFI 778bn plus CBC securities 554.7bn plus banks 199bn against a TIC row of 677bn — so **TIC both mixes holders and undercounts Taiwan**, the Formosa book and custodial attribution being the two reasons. Lifer claims therefore rest on series 9 (sector-identified, reserves excluded by construction) and series 11. **The same template returns the CBC swap book**, which Setser & S.T.W. could only estimate at USD 130bn (60–200bn interval): it is **USD 91.9bn in 2021Q4 falling to 76.1bn in 2026Q1**, at the low end and shrinking — a direct input to series 8. **Read the correction in 4.42 before using TIC levels behaviourally:** TIC holdings are at MARKET VALUE, so a change in level mixes price with flow. The BoP transactions series shows the sector still a net buyer of long-term foreign debt in every year 2019–2024 (~USD 147bn cumulative), so the agency-MBS decline is rotation into corporate credit plus valuation, **not liquidation** — 4.40 read it as a retreat and that was wrong. The genuine net selling is a 2025 event visible in the BoP, and the June-2025 TIC survey shows Taiwan's US long-term debt stock still RISING to 677bn through the shock quarter, with the CBC's reserve build in Treasuries (+39bn) and the lifers still adding corporates (+13bn) while agency MBS shrank again (−8bn). Where the two disagree, the BoP answers behaviour and TIC answers composition. Treasuries and agency non-ABS mix the CBC's reserves with private money and are deliberately NOT attributed to the lifers. Validation is exact: every row sums to its printed total, and Table A3/A9 agrees with the composition table on 11 of 11 overlapping years. **And the USD book the TIC cannot see is now sized (4.41):** the lifers' own foreign debt stock exceeds Taiwan's entire country-wide holding of US long-term debt from 2019, and the Formosa market is why — TPEx's register shows **USD 218bn of USD-denominated 國際債券, amount-weighted original tenor 28.0 years, 74.5% with 20+ years still to run, 75.9% issuer-callable** on the classic 5-year non-call, annually callable structure. Both legs of the lifers' USD duration bid, the 192bn of agency MBS and the 166bn of callable Formosa, are short convexity. Issuers are Gulf sovereigns, global bank holdcos and US telecoms — long-dated USD corporate and quasi-sovereign credit, exactly the segment the research question named. The register carries no holder information, so **a sourced series for lifer holdings of 國際板債券 is the named open gap**; nothing loaded attributes the market to the lifers. Stages 6–7 otherwise not started. **A per-firm rate/FX sensitivity panel is in progress:** the insurers' statutory filings print 利率風險敏感度分析表 (change in P&L and equity per 1bp by currency — a disclosed DV01) and 匯率風險敏感度分析表 (per 1% FX move, including the FX reserve column). Cathay Life's USD equity DV01 was NT$2.53bn per bp at 2026Q1 against NT$1.38bn a year earlier — nearly doubled, alongside a strategy deck describing "redesignation of AC assets to FVOCI", so the series measures disclosed balance-sheet sensitivity and never economic duration. The parser is written and verified on that filing; the MOPS pull is throttled by the WAF and running slowly in the background.
