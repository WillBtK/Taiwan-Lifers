...
| `duration_gap_composite.csv` | JLIM's sector duration gap: the four-firm EEV composite (Dai-ichi, Meiji Yasuda, Sumitomo, Japan Post) FY2008–FY2024 in years of duration, negative meaning liabilities are more rate-sensitive than assets, plus the FY2025 seven-firm statutory cross-section that JLIM shows dashed and calls provisional. Copied from the data blob of JLIM's published chart pack (`charts/jlim_ledger.html`, `gap.aggregate` and `gap.sector.withJpiYearsGap`), which is built from `duration_firm_annual.csv` (also copied). | `build/layer2.py`, `build/inversion.py`, `charts/jlim_ledger.html` |
| `duration_firm_annual.csv` | The firm-level EEV-inversion duration gaps behind the composite. Copied for traceability; not read by the page. | `build/layer2.py` |
