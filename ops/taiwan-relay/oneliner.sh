...
URL=$(gcloud run services describe taiwan-relay --region asia-east1 --format='value(status.url)')
...
if [ "$code" = "200" ]; then echo "PASS - add the two values above as GitHub secrets."; else echo "FAIL ($code) - Cloud Run egress is not read as Taiwan; the VM fallback is needed."; fi
